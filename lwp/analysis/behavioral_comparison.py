...
def aggregate_model_distribution(histograms: list[dict]) -> dict:
    ...
    main_total = np.sum(
        [h["main_thrust_counts"] for h in histograms], axis=0
    ).astype(float)
    side_total = np.sum(
        [h["side_thrust_counts"] for h in histograms], axis=0
    ).astype(float)
...
def compute_adaptation_score(
    ...
    df = episodes_df.copy()
    df["_npz_basename"] = df["npz_path"].apply(
    ...
    for q in range(n_quartiles):
        ...
        if q_hists:
            ...
            n_bins = len(
                next(iter(histograms.values()))["main_thrust_counts"]
            )
            q_dist = {
    ...
    adaptation_score = (
        float(pairwise.sum() / (2 * n_pairs)) if n_pairs > 0 else 0.0
    )
...
def compute_binned_performance(
    ...
    df["_bin"], bin_edges = pd.cut(
        df[bin_col], bins=n_bins, retbins=True, include_lowest=True,
    )
