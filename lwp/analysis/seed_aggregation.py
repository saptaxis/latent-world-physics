...
from lwp.analysis.tb_parser import (
...
def aggregate_seed_metrics(
    ...
    for seed in seeds:
        per_seed_finals[seed] = extract_last_k_metrics(
            all_scalars[seed], tags=all_tags, last_k=last_k,
        )
    ...
    for tag in EVAL_METRIC_TAGS:
        ...
        if valid:
            training_metrics[name] = {
                ...
                "std": round(float(np.std(valid, ddof=1)) if len(valid) > 1 else 0.0, 2),
    ...
    for tag in TRAIN_METRIC_TAGS:
        ...
        if valid:
            training_dynamics[name] = {
                ...
                "std": round(float(np.std(valid, ddof=1)) if len(valid) > 1 else 0.0, 4),
    ...
    for tag in CURVE_METRIC_TAGS:
        ...
        if common_steps:
            ...
            curves[name] = {
                ...
                "std": [round(float(v), 4) for v in np.nanstd(values_matrix, axis=0, ddof=1)]
                    if len(seeds) > 1 else [0.0] * len(common_steps),
                "per_seed": per_seed,
            }
        else:
...
def write_experiment_summary(
    ...
    fieldnames = [
        "config", "n_seeds",
        "landed_pct_mean", "landed_pct_std",
        "mean_reward_mean", "mean_reward_std",
        "crashed_pct_mean", "timeout_pct_mean",
    ]
    rows = []
    for config_name, result in sorted(configs_results.items()):
        fm = result.get("training_metrics", {})
    ...
    for metric_name, ylabel, base_color in eval_metrics:
        ...
        for seed in seeds:
            ...
            if seed_key in per_seed:
                ax.plot(metric_steps, per_seed[seed_key],
                        linewidth=0.8, alpha=0.4, color=base_color)

        # Aggregated mean.
        if mc.get("mean"):
            ax.plot(metric_steps, mc["mean"],
                    linewidth=2.0, color=base_color, label="mean")
...
def plot_summary(
    ...
    eval_panels = [(k, l, c, yl) for k, l, c, yl in eval_panels if k in training_metrics]
    if training_dynamics:
        dynamics_panels = [(k, l, c, yl) for k, l, c, yl in dynamics_panels if k in training_dynamics]
    else:
        dynamics_panels = []
    ...
    for ax, (metric_name, label, color, ylim) in zip(eval_axes, eval_panels):
        _draw_dot_panel(ax, training_metrics[metric_name], seeds, label, color, ylim=ylim)

    # --- Draw dynamics panels ---
    if has_dynamics:
        for ax, (metric_name, label, color, ylim) in zip(dyn_axes, dynamics_panels):
            _draw_dot_panel(ax, training_dynamics[metric_name], seeds, label, color, ylim=ylim)
...
def _draw_dot_panel(ax, metric_data: dict, seeds: list[int],
                    label: str, color: str,
    ...
    if valid_points:
        ...
        ax.scatter(jittered_x, ys, s=80, c=color, alpha=0.7,
                   edgecolors="white", linewidth=0.5, zorder=3)

        # Label each point with seed number.
        for jx, y, idx in zip(jittered_x, ys, xs):
            ax.annotate(f"s{seeds[idx]}", (jx, y),
                        textcoords="offset points", xytext=(8, 0),
                        fontsize=7, color="gray")

    # Mean as horizontal line.
    if mean_val is not None:
        ax.axhline(mean_val, color=color, linewidth=1.5, linestyle="--",
                   alpha=0.8, label=f"mean={mean_val:.4g}")
