...
_CONFIGS_DIR = Path(__file__).parent.parent.parent / "configs" / "agents"
_BATCHES_DIR = Path(__file__).parent.parent.parent / "batches"
...
TRAINING_DEFAULTS = {
    ...
    # Override rl_common PPO_DEFAULTS / SAC_DEFAULTS. None = use rl_common defaults.
    # These are passed as algo_kwargs to train().
    # Visual (CNN) agents need very different PPO params than state-vector agents:
    #   State-vector (RL Zoo LunarLander): n_steps=1024, batch_size=64, n_epochs=4
    #   Visual (RL Zoo Atari):             n_steps=128,  batch_size=256, n_epochs=4
    "ent_coef": None,        # entropy coefficient (PPO default: 0.01, SAC: "auto")
    "learning_rate": None,   # e.g., 3e-4 (state), 2.5e-4 (visual)
    "n_steps": None,         # rollout buffer per env (1024 state, 128 visual)
    "batch_size": None,      # SGD mini-batch size (64 state, 256 visual)
    "n_epochs": None,        # SGD passes per update (4 is standard)
    "gamma": None,           # discount factor (0.999 state, 0.99 visual)
    "gae_lambda": None,      # GAE lambda (0.98 state, 0.95 visual)
    "clip_range": None,      # PPO clip range (0.2 state, 0.1 visual)
    "max_grad_norm": None,   # gradient clipping (0.5 is standard)
    "lr_schedule": None,     # None = constant, "linear" = decay to 0 over total_steps

    # --- Reward shaping ---
    "angle_shaping_omega": None,  # Extra angle delta reward weight. None = disabled.
                                   # 200.0 = triples angle signal (base env uses 100).
    "time_penalty": None,          # Per-step negative reward. None = disabled.
                                   # 0.5 = -0.5 per step (600-step ep costs -300 extra).

    # --- Auxiliary losses ---
    "aux_kinematic": False,   # Add aux head predicting kinematic state from CNN features
    "aux_coef": 0.5,          # Weight of aux loss (0.5 = half the PPO loss magnitude)

    # --- Visual ---
    "n_stack": 4,            # frame stack depth (visual variants only)
    "frame_size": 84,        # pixel resolution (84 standard, 128 for more detail)
    "share_features_extractor": True,  # True = shared CNN for actor+critic (SB3 default)
                                        # False = separate CNNs, no gradient interference
    "cnn_backbone": "nature",   # "nature" = NatureCNN (SB3 default), "impala" = IMPALA ResNet
    "cnn_channels": None,        # IMPALA channel widths (default [16,32,32]). E.g. [32,64,64] for wider.
    "cnn_pool_size": None,       # IMPALA adaptive pool size (default 4). 8 = more spatial info.
    "cnn_features_dim": None,    # IMPALA output dim (default 256). 512 = more capacity.
...
def _resolve_path(name_or_path: str, builtin_dir: Path, kind: str) -> Path:
    ...
    raise FileNotFoundError(
        f"No builtin {kind} '{name_or_path}'. "
