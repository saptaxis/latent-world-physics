...
def select_episodes(
    ...
    if diversity_on and diversity_on in filtered.columns and len(filtered) >= n:
        ...
        filtered["_bin"] = pd.qcut(
            filtered[diversity_on], q=min(n, len(filtered)),
...
def record_clip(
    ...
    from parametric_lunar_lander.wrappers import PhysicsBlindWrapper, RaycastWrapper
    ...
    if use_heuristic:
        ...
        if vec_norm_path:
            ...
            result = _run_episode_vec(
                vec_env, model, base_env, seed, max_steps,
                physics_config,
            )
            vec_env.close()
    ...
    result = run_episode(
        env=env, policy_fn=policy_fn, seed=seed,
        max_steps=max_steps, save_frames=True,
    )
...
def render_clean_clip(
    ...
    if len(actions) > 1:
        ...
        behavioral["thrust_autocorr_lag1"] = float(
            np.corrcoef(main_thrust[:-1], main_thrust[1:])[0, 1]
        ) if len(main_thrust) > 2 else 0.0
        behavioral["thrust_duty_cycle"] = float(
            np.mean(main_thrust > 0.05)
        )
        behavioral["fuel_efficiency"] = (
            float(ep["metadata"].get("total_reward", 0))
