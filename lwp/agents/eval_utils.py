...
def resolve_model_path(checkpoint_dir: str, model_name: str | None = None) -> str:
    ...
    if model_name:
        if os.path.isabs(model_name):
            ...
            return model_name
        if os.sep in model_name or "/" in model_name:
            # Relative path like "best/model.zip" — resolve against checkpoint_dir
            relative_path = os.path.join(checkpoint_dir, model_name)
...
def resolve_vec_normalize_path(
    checkpoint_dir: str, model_path: str,
) -> str | None:
    ...
    if model_basename.startswith("rl_model_") and model_basename != "rl_model.zip":
        vec_norm_name = (model_basename
                         .replace("rl_model_", "rl_model_vecnormalize_")
                         .replace(".zip", ".pkl"))
...
def make_env_factory(
    ...
    frame_size: int = 84,
    angle_shaping_omega: float | None = None,
    time_penalty: float | None = None,
    aux_kinematic: bool = False,
    ...
    def env_fn(seed):
        return make_lunar_lander_env(
            ...
            angle_shaping_omega=angle_shaping_omega,
            time_penalty=time_penalty,
            aux_kinematic=aux_kinematic,
        )
...
def build_eval_batches(
    ...
    if profiles_str:
        ...
        for prof_name in profiles_str.split(","):
            ...
            fn = make_env_factory(
                variant=variant, n_rays=n_rays,
                history_k=history_k, profile=prof_name,
...
def compute_episode_metrics(record: dict, episode_idx: int = 0,
...
def evaluate_agent(
    ...
    n_envs: int = 1,
    profile: str | None = None,
    n_stack: int = 0,  # frame stacking for visual agents
) -> dict:
    ...
    Args:
        ...
        n_stack: Number of frames to stack for visual agents (0 = no stacking).
    ...
    # Frame stacking for visual agents (must be before VecNormalize).
    if n_stack > 0:
        ...
        while len(episodes) < n_episodes:
            ...
            if done[0]:
                record = {
                    "reward": ep_reward,
                    "steps": ep_steps,
                    "info": {k: v for k, v in infos[0].items()
                             if k != "terminal_observation"},
                }
                metrics = compute_episode_metrics(
                    record, episode_idx=len(episodes), profile=profile,
                )
            ...
            for i in range(n_envs):
                ...
                if dones[i] and len(episodes) < n_episodes:
                    record = {
                        "reward": ep_rewards[i],
                        "steps": ep_steps[i],
                        "info": {k: v for k, v in infos[i].items()
                                 if k != "terminal_observation"},
                    }
                    metrics = compute_episode_metrics(
                        record, episode_idx=len(episodes), profile=profile,
...
def plot_eval_summary(episodes: list[dict], output_dir: str,
                      per_profile_summaries: dict | None = None):
    ...
    bars = ax.bar(shown.keys(), shown.values(),
                  color=[colors[k] for k in shown.keys()])
    ...
    if groups_with_data:
        bp = ax.boxplot(groups_with_data, tick_labels=labels_with_data, patch_artist=True)
    ...
    if twrs:
        ...
        y_positions = {"crashed": 0.0, "out_of_bounds": 0.33, "timeout": 0.66, "landed": 1.0}
        for oc in VALID_OUTCOMES:
            vals = [e["twr"] for e in episodes
            ...
            if vals:
                ...
                ax.scatter(vals, [y_pos + j for j in jitter],
                           alpha=0.4, s=15, color=colors[oc], label=oc)
        ax.set_xlabel("Thrust-to-Weight Ratio")
        shown_outcomes = [oc for oc in VALID_OUTCOMES
                          if any(e["outcome"] == oc for e in episodes)]
    ...
    if per_profile_summaries and len(per_profile_summaries) > 1:
        ...
        for i, (oc, color) in enumerate([
            ...
            if any(v > 0 for v in pcts):
                ax.bar(x + (i - 1.5) * width, pcts, width,
                       label=oc.replace("_", " ").title(), color=color)
        ax.set_xticks(x)
