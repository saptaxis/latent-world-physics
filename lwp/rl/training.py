...
SAC_DEFAULTS = dict(
    learning_rate=3e-4,
    buffer_size=100_000,    # replay buffer — 100K (image obs: 100K × 128×128×4 × 2 ≈ 13 GB)
    ...
    # optimize_memory_usage=True would halve buffer RAM but conflicts with
    # handle_timeout_termination (SB3 default). Use smaller buffer instead.
)
...
class VideoRecorderCallback(BaseCallback):
    ...
    Uses record_episode_videos() from lwp.rl.inference to capture frames
    ...
    def _on_step(self) -> bool:
        ...
        from lwp.rl.inference import record_episode_videos
        ...
        if self.verbose:
            print(f"[Video] step={self.num_timesteps}: {len(results)} episodes, "
...
def train(
    ...
    encoder_weights: str | None = None,   # path to pre-trained encoder .pt
    freeze_encoder: bool = False,          # freeze encoder during RL
    ...
    3. Creates a PPO or SAC agent with configurable policy class
    ...
    Args:
        ...
        policy_class: SB3 policy string — "MlpPolicy" for state-vector obs,
            "CnnPolicy" for pixel/image obs. Default "MlpPolicy" preserves
            backward compatibility.
        norm_obs: Whether to normalize observations via VecNormalize. Should
            be True for state vectors (different scales need normalization)
            and False for pixel observations (CNN expects raw uint8 pixels).
        n_stack: Number of frames to stack via VecFrameStack. 0 means no
            frame stacking. Typical values: 4 for Atari-style visual obs.
            Stacking is applied after VecMonitor and before VecNormalize.
    ...
    # DummyVecEnv for single env (avoids subprocess serialization issues
    # with image obs in off-policy algorithms like SAC).
    # SubprocVecEnv for multiple envs (parallel rollout collection for PPO).
    env_fns = [_make_env_thunk(env_fn, seed + i) for i in range(n_envs)]
    if n_envs == 1:
        train_vec_env = DummyVecEnv(env_fns)
    else:
        train_vec_env = SubprocVecEnv(env_fns)
    ...
    # Frame stacking for visual observations. Applied before VecNormalize
    # so the stacked frames are what gets (optionally) normalized.
    if n_stack > 0:
    ...
    # rewards. For state-vector obs (MlpPolicy), norm_obs=True is critical
    # because the state has wildly different scales: position (0-3000px),
    # velocity (-500..500px/s), physics params (60-200), and binary flags
    # (0/1). For pixel obs (CnnPolicy), norm_obs=False because the CNN
    # expects raw uint8 pixels — normalizing them would destroy the spatial
    # structure the CNN relies on.
    # Skip VecNormalize for off-policy visual: SAC stores unnormalized obs
    # in the replay buffer via VecNormalize.get_original_obs(), which returns
    # channels-last (pre-VecTransposeImage). The replay buffer expects
    # channels-first, causing a shape mismatch. Since norm_obs=False for
    # visual anyway and SAC has auto entropy tuning, VecNormalize is unnecessary.
    skip_vec_normalize = (algo == "sac" and not norm_obs)
    if not skip_vec_normalize:
        train_vec_env = VecNormalize(
            train_vec_env,
            norm_obs=norm_obs,       # False for visual (CNN expects raw pixels)
            norm_reward=True,
    ...
    pk = policy_kwargs if policy_kwargs is not None else DEFAULT_POLICY_KWARGS.copy()
    if algo == "ppo":
        defaults = PPO_DEFAULTS.copy()
        defaults.update(algo_kwargs or {})
        PPOClass = algo_class_override if algo_class_override is not None else PPO
        model = PPOClass(
            policy_class,
            train_vec_env,
            policy_kwargs=pk,
    ...
    # If encoder_weights is provided, load the state_dict into the CNN
    # encoder. This replaces random init with weights pre-trained on
    # kinematic prediction or reconstruction (see pretrain_encoder.py).
    #
    # Two cases for where the CNN lives:
    # - CnnPolicy (visual-blind): features_extractor IS the CNN
    # - MultiInputPolicy (visual-labeled): features_extractor is a
    #   CombinedExtractor with the CNN at extractors["image"]
    if encoder_weights is not None:
        import torch as th
        print(f"Loading pre-trained encoder weights from {encoder_weights}")
        state_dict = th.load(encoder_weights, weights_only=True, map_location="cpu")

        # Find the CNN module(s) to load weights into. Three cases:
        # 1. PPO CnnPolicy: policy.features_extractor IS the CNN
        # 2. PPO MultiInputPolicy: policy.features_extractor.extractors["image"]
        # 3. SAC: policy.features_extractor is None — CNN lives on
        #    actor.features_extractor and critic.features_extractor (separate copies)
        def _find_cnn(fe):
            """Extract the CNN from a features_extractor (handles CombinedExtractor)."""
            if fe is None:
                return None
            if hasattr(fe, "extractors") and "image" in fe.extractors:
                return fe.extractors["image"]
            return fe

        # Collect all CNN modules to load into.
        cnn_modules = []
        fe = model.policy.features_extractor
        if fe is not None:
            cnn_modules.append(("policy.features_extractor", _find_cnn(fe)))
        else:
            # SAC: separate actor/critic feature extractors.
            if hasattr(model, "actor") and hasattr(model.actor, "features_extractor"):
                cnn_modules.append(("actor.features_extractor", _find_cnn(model.actor.features_extractor)))
            if hasattr(model, "critic") and hasattr(model.critic, "features_extractor"):
                cnn_modules.append(("critic.features_extractor", _find_cnn(model.critic.features_extractor)))
            # SAC also has a critic_target with its own copy.
            if hasattr(model, "critic_target") and hasattr(model.critic_target, "features_extractor"):
                cnn_modules.append(("critic_target.features_extractor", _find_cnn(model.critic_target.features_extractor)))

        if not cnn_modules:
            raise RuntimeError("Could not find any features_extractor to load encoder weights into")

        for name, cnn in cnn_modules:
            cnn.load_state_dict(state_dict)
            print(f"  Loaded into {name}")

        n_loaded = sum(p.numel() for p in cnn_modules[0][1].parameters())
        print(f"  {len(state_dict)} weight tensors × {len(cnn_modules)} modules ({n_loaded:,} params each)")

        if freeze_encoder:
            print("  Freezing encoder (requires_grad=False for all CNN params)")
            for name, cnn in cnn_modules:
            ...
            print(f"  Trainable after freeze: {n_trainable:,} params")
    ...
    # --- Save architecture ---
    # Dump the full policy architecture (layers, shapes, param counts) to a
    # text file in the run directory. Makes it easy to visually verify what
    # was actually built — catches net_arch / policy_kwargs misconfigurations.
    arch_str = str(model.policy)
    n_params = sum(p.numel() for p in model.policy.parameters())
    n_trainable = sum(p.numel() for p in model.policy.parameters() if p.requires_grad)
    # Include aux head if present (e.g., AuxPPO's kinematic prediction head).
    aux_str = ""
    if hasattr(model, "aux_head"):
        aux_params = sum(p.numel() for p in model.aux_head.parameters())
        aux_str = (
            f"\n--- Auxiliary Head ---\n"
            f"Parameters: {aux_params:,}\n"
            f"{model.aux_head}\n"
        )
        n_params += aux_params
        n_trainable += sum(p.numel() for p in model.aux_head.parameters() if p.requires_grad)

    arch_summary = (
        f"Policy: {policy_class}\n"
        f"Algorithm: {algo.upper()}\n"
        f"Parameters: {n_params:,} total, {n_trainable:,} trainable\n"
        f"policy_kwargs: {pk}\n"
        f"\n{arch_str}\n"
        f"{aux_str}"
    )
    arch_path = os.path.join(checkpoint_dir, "architecture.txt")
