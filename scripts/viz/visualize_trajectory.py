...
def render_annotated_frame(
    ...
    if _gravity > 0 and _density > 0:
        ...
        twr_color = (100, 255, 100) if twr >= 5.0 else (255, 200, 100) if twr >= 2.0 else (255, 100, 100)
        draw.text((x0, y), f"  {'TWR':>6s}: {twr:8.1f}", fill=twr_color, font=small_font)
    else:
    ...
    y += line_h
    draw.text((x0, y), f"  pos: ({state[0]:+.2f}, {state[1]:+.2f})", fill=text_color, font=small_font)
    y += small_line_h
    draw.text((x0, y), f"  vel: ({state[2]:+.2f}, {state[3]:+.2f})", fill=text_color, font=small_font)
    y += small_line_h
    draw.text((x0, y), f"  angle: {state[4]:+.3f} rad", fill=text_color, font=small_font)
    y += small_line_h
    ...
    draw.text((leg_x + small_font.getlength("L "), y), "R", fill=right_color, font=small_font)
    ...
    if action is not None:
        ...
        draw.text((x0, y), f"  main: {_action_bar(main_val)}", fill=main_color, font=small_font)
        y += small_line_h
        ...
        draw.text((x0, y), f"  side: {side_dir} {_action_bar(side_val)}", fill=side_color, font=small_font)
        y += small_line_h
    else:
    ...
    y += line_h
    r_color = (
        (100, 255, 100) if reward > 0.5 else
        (255, 100, 100) if reward < -0.5 else
        text_color
    )
    draw.text((x0, y), f"  step:  {reward:+.1f}", fill=r_color, font=small_font)
    y += small_line_h
    draw.text((x0, y), f"  total: {cumulative_reward:+.1f}", fill=text_color, font=small_font)
    y += small_line_h + section_gap
...
def visualize_episode(npz_path, output_path, scale=1, fps=50, annotate=True):
    ...
    print(f"Rendering {npz_path.name}: {n_steps} steps, "
    ...
    iio.imwrite(
        ...
        fps=fps, codec="libx264", macro_block_size=1,
    )
...
def main():
    ...
    if len(input_paths) == 1 and input_paths[0].is_dir():
        ...
        if args.sample:
            ...
            indices = rng.choice(len(all_npz), size=min(args.sample, len(all_npz)), replace=False)
    ...
    for i, npz_path in enumerate(input_paths):
        if not npz_path.exists():
            ...
            sys.exit(1)
        output = args.output if len(input_paths) == 1 and args.output else str(npz_path.with_suffix(".mp4"))
        visualize_episode(
            npz_path, output,
            scale=args.scale, fps=args.fps,
            annotate=not args.no_annotations,
