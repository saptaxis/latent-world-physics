...
Usage:
    ...
    python lunar_lander/scripts/train_all_agents.py \\
        ...
        --output-base /media/hdd1/.../rl_agents/multi-seed
"""
...
def main():
    ...
    parser.add_argument("--resume", action="store_true",
                        help="Pass --resume to each training run")
    ...
    print("=" * 60)
    if seeds:
        print(f"Batch: {args.batch} — {len(runs)} configs × {len(seeds)} seeds "
              f"= {len(matrix)} runs")
        print(f"Seeds: {seeds}")
    ...
    print(f"\n{'='*60}")
    print(f"Batch complete: {completed}/{len(matrix)} succeeded"
