...
from lwp.agents.eval_utils import (
    evaluate_agent, compute_summary, plot_eval_summary,
...
from scripts.viz.visualize_trajectory import visualize_episode
...
def _record_annotated_videos(
    ...
    # Stack: VecNormalize -> VecFrameStack -> DummyVecEnv -> [wrappers] -> base
    # Peel through VecEnvWrapper layers until we hit DummyVecEnv.
    inner = vec_env
...
def main():
    ...
    parser.add_argument("--n-envs", type=int, default=1,
                        help="Parallel env workers (default: 1)")
    ...
    parser.add_argument("--record-videos", action="store_true",
                        help="Record annotated mp4 videos")
    parser.add_argument("--n-videos", type=int, default=5,
                        help="Number of video episodes (default: 5)")
    ...
    n_stack = train_config.get("n_stack", 0) if is_visual else 0
    frame_size = train_config.get("frame_size", 84)

    print(f"Loaded config: {variant} / {algo.upper()} / "
    ...
    eval_batches = build_eval_batches(
        ...
        profiles_str=args.profiles, default_env_fn=env_fn,
        frame_size=frame_size,
    )
    ...
    print(f"  Mean reward:    {overall_summary['mean_reward']:>8.1f} "
          f"+/- {overall_summary['std_reward']:.1f}")
    print(f"  Mean steps:     {overall_summary['mean_steps']:>8.1f}")
    print()
    print(f"  Outcomes:")
    print(f"    Landed:       {overall_summary['n_landed']:>3d}/"
          f"{overall_summary['n_episodes']} "
          f"({overall_summary['landed_pct']:.0f}%)")
    print(f"    Crashed:      {overall_summary['n_crashed']:>3d}/"
          f"{overall_summary['n_episodes']} "
          f"({overall_summary['crashed_pct']:.0f}%)")
    print(f"    Out of bounds:{overall_summary['n_out_of_bounds']:>3d}/"
          f"{overall_summary['n_episodes']} "
          f"({overall_summary['out_of_bounds_pct']:.0f}%)")
    print(f"    Timeout:      {overall_summary['n_timeout']:>3d}/"
          f"{overall_summary['n_episodes']} "
    ...
    if len(per_profile_summaries) > 1:
        ...
        for prof_name, s in per_profile_summaries.items():
            print(f"    {prof_name:12s}: {s['n_landed']}/{s['n_episodes']} landed "
                  f"({s['landed_pct']:.0f}%), "
    ...
    if args.output_dir:
        ...
        plot_eval_summary(
            all_episodes, args.output_dir,
            per_profile_summaries=(per_profile_summaries
                                   if len(per_profile_summaries) > 1 else None),
        )
    ...
    if args.record_videos:
        ...
        for vr in video_results:
            print(f"    {os.path.basename(vr['path']):25s}  "
