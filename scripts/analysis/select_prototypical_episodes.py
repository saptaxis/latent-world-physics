...
from lwp.agents.clip_recording import select_episodes
...
def _parse_filter(filter_str: str) -> tuple[str, tuple[float | None, float | None]]:
    ...
    raise ValueError(f"Cannot parse filter: {filter_str}. Use column>=val or column<=val.")
...
def main():
    ...
    parser.add_argument("--descending", action="store_true", help="Sort descending (default)")
    ...
    results = select_episodes(
        df, n=args.n, outcome=args.outcome,
        sort_by=args.sort_by, ascending=ascending,
    ...
    for r in results:
        print(f"  {Path(r['npz_path']).name}: {r['outcome']}, "
