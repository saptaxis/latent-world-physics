...
from lwp.analysis.seed_aggregation import (
...
def main():
    ...
    if args.config:
        if args.config not in configs:
            print(f"ERROR: Config '{args.config}' not in manifest. "
                  f"Available: {list(configs.keys())}", file=sys.stderr)
            sys.exit(1)
    ...
    for config_name, config_data in sorted(configs.items()):
        ...
        print(f"    landed={landed.get('mean', '?')} +/- {landed.get('std', '?')}  "
