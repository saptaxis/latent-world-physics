...
from lwp.analysis.behavioral_metrics import (
...
from lwp.analysis.trajectory_metrics import (
...
def _plot_adaptation_by_twr(adaptation_result: dict, output_path: Path):
    ...
    for i, (q_dist, (lo, hi), count, color) in enumerate(
        ...
        ax.step(centers, q_dist["main_thrust_probs"],
                where="mid", alpha=0.8, color=color, label=label,
                linewidth=1.5)

    ax.set_xlabel("Main Thrust")
    ax.set_ylabel("Probability")
    ax.set_title(
        f"P(main_thrust) by TWR Quartile — Adaptation Score: {score:.4f}"
    )
    ax.set_xlim(0, 1)
...
def _plot_landed_vs_twr(binned_perf: pd.DataFrame, output_path: Path):
    ...
    ax.set_xticklabels(
        binned_perf["bin_label"], rotation=30, ha="right", fontsize=8,
    )
...
_VEHICLE_PARAMS = ["main_engine_power", "side_engine_power",
                   "lander_density", "angular_damping"]
...
def _plot_physics_distribution(episodes_df: pd.DataFrame, output_path: Path):
    ...
    def _plot_group(ax, param_names, title):
        ...
        for param_idx, param in enumerate(param_names):
            ...
            if len(unique_bounds) == 1:
                ...
                ax.fill_between(
                    ...
                    nlo, nhi, alpha=0.08, color="gray", linewidth=0,
                )
                ax.text(param_idx, 0.5, "TWR\nfiltered", ha="center", va="center",
                        fontsize=6, color="silver", fontstyle="italic")
            else:
                ...
                for difficulty in ["hard", "medium", "easy"]:
                    ...
                    ax.fill_between(
                        [param_idx - 0.4, param_idx + 0.4],
                        nlo, nhi, alpha=0.18, color=color, linewidth=0,
                    )
        ...
        for param_idx, param in enumerate(param_names):
            ...
            lo_str = f"{lo:g}"
            hi_str = f"{hi:g}"
            ax.text(param_idx, -0.08, lo_str, ha="center", va="top",
                    fontsize=7, color="gray")
            ax.text(param_idx, 1.08, hi_str, ha="center", va="bottom",
                    fontsize=7, color="gray")
...
def main():
    ...
    parser.add_argument(
        "--agent-dir", required=True, type=str,
    ...
    output_dir = (
        ...
        else agent_dir / "behavioral_analysis"
    )
    ...
    histograms = compute_collection_histograms(
        str(traj_dir), workers=args.workers, n_bins=args.n_bins,
    )
    ...
    if metrics_path.exists():
        ...
        episodes_df = compute_collection_metrics(
            str(traj_dir), workers=args.workers,
        )
    ...
    print(
        f"  {len(episodes_df)} episodes, {n_landed} landed ({landed_pct:.1f}%)"
    )
    ...
    adaptation_result = compute_adaptation_score(
        histograms, episodes_df,
        physics_col="twr", n_quartiles=args.n_quartiles,
    )
    print(f"  Adaptation score: {adaptation_result['adaptation_score']:.4f}")
    for i, ((lo, hi), count) in enumerate(
        zip(adaptation_result["quartile_boundaries"],
    ...
    binned_perf = compute_binned_performance(
        episodes_df, bin_col="twr", n_bins=args.twr_bins,
    )
    ...
    _plot_action_distributions(
        model_dist, output_dir / "action_distributions.png",
    )
    _plot_adaptation_by_twr(
        adaptation_result, output_dir / "adaptation_by_twr.png",
    )
    ...
    summary = {
        ...
        "quartile_episode_counts": adaptation_result[
            "quartile_episode_counts"
        ],
